...
{
    printk("+++++csky_crypto_crc_init_hw key is %d mod is %d std is %d\n", ctx->key, ctx->mod, ctx->std);
	writel(ctx->key, &crc->regs->config_reg);
...
static int csky_crypto_crc_handle_sg(struct csky_crypto_crc *crc)
{
	...
	for (i = 0; i < nsg; i++) {
		...
		writel(*(u32 *)ctx->bufnext, &crc->regs->new_data);
...
static int csky_crypto_crc_handle(struct csky_crypto_crc *crc)
{
	...
	put_unaligned_le32(readl(&crc->regs->result), req->result);
...
static int csky_crypto_crc16ccitt_flase_cra_init(struct crypto_tfm *tfm)
{
	struct csky_crypto_crc_ctx *crc_ctx = crypto_tfm_ctx(tfm);
	int ret = 0;
...
static int csky_crypto_crc16xmodem_cra_init(struct crypto_tfm *tfm)
{
	struct csky_crypto_crc_ctx *crc_ctx = crypto_tfm_ctx(tfm);
	int ret = 0;
...
static int csky_crypto_crc16dnp_cra_init(struct crypto_tfm *tfm)
{
	struct csky_crypto_crc_ctx *crc_ctx = crypto_tfm_ctx(tfm);
	int ret = 0;
...
static int csky_crypto_crc16itu_cra_init(struct crypto_tfm *tfm)
{
	struct csky_crypto_crc_ctx *crc_ctx = crypto_tfm_ctx(tfm);
	int ret = 0;
...
static int csky_crypto_crc16none_cra_init(struct crypto_tfm *tfm)
{
	struct csky_crypto_crc_ctx *crc_ctx = crypto_tfm_ctx(tfm);
	int ret = 0;
...
static int csky_crypto_crc16pmeq_cra_init(struct crypto_tfm *tfm)
{
	struct csky_crypto_crc_ctx *crc_ctx = crypto_tfm_ctx(tfm);
	int ret = 0;
...
static struct ahash_alg crc_algs[] = {
...

	{
		...
		.halg.statesize		= sizeof(struct csky_crc_reqctx),
		.halg.base  = {
			.cra_name	= "crc16_ccitt_flase",
			.cra_driver_name= "csky-crc16-ccitt-flase",
			.cra_priority	= 100,
			.cra_flags	= CRYPTO_ALG_ASYNC,
			.cra_blocksize  = CHKSUM_BLOCK_SIZE,
			.cra_ctxsize	= sizeof(struct csky_crypto_crc_ctx),
			.cra_alignmask  = 0,
			.cra_module	= THIS_MODULE,
			.cra_init	= csky_crypto_crc16ccitt_flase_cra_init
		}
	},

	{
		...
		.halg.statesize		= sizeof(struct csky_crc_reqctx),
		.halg.base  = {
			.cra_name	= "crc16_xmodem",
			.cra_driver_name= "csky-crc16-xmodem",
			.cra_priority	= 100,
			.cra_flags	= CRYPTO_ALG_ASYNC,
			.cra_blocksize  = CHKSUM_BLOCK_SIZE,
			.cra_ctxsize	= sizeof(struct csky_crypto_crc_ctx),
			.cra_alignmask  = 0,
			.cra_module	= THIS_MODULE,
			.cra_init	= csky_crypto_crc16xmodem_cra_init
		}
	},

	{
		...
		.halg.base  = {
			.cra_name	= "crc16_dnp",
			.cra_driver_name= "csky-crc16-dnp",
			.cra_priority	= 100,
			.cra_flags	= CRYPTO_ALG_ASYNC,
			.cra_blocksize  = CHKSUM_BLOCK_SIZE,
			.cra_ctxsize	= sizeof(struct csky_crypto_crc_ctx),
			.cra_alignmask  = 0,
			.cra_module	= THIS_MODULE,
			.cra_init	= csky_crypto_crc16dnp_cra_init
		}
	},

	{
		...
		.halg.base  = {
			.cra_name	= "crc16_itu",
			.cra_driver_name= "csky-crc16-itu",
			.cra_priority	= 100,
			.cra_flags	= CRYPTO_ALG_ASYNC,
			.cra_blocksize  = CHKSUM_BLOCK_SIZE,
			.cra_ctxsize	= sizeof(struct csky_crypto_crc_ctx),
			.cra_alignmask  = 0,
			.cra_module	= THIS_MODULE,
			.cra_init	= csky_crypto_crc16itu_cra_init
		}
	},

	{
		...
		.halg.base  = {
			.cra_name	= "crc16_pmeq",
			.cra_driver_name= "csky-crc16-pmeq",
			.cra_priority	= 100,
			.cra_flags	= CRYPTO_ALG_ASYNC,
			.cra_blocksize  = CHKSUM_BLOCK_SIZE,
			.cra_ctxsize	= sizeof(struct csky_crypto_crc_ctx),
			.cra_alignmask  = 0,
			.cra_module	= THIS_MODULE,
			.cra_init	= csky_crypto_crc16pmeq_cra_init
		}
	},

	{
		...
		.halg.base  = {
			.cra_name	= "crc16_none",
			.cra_driver_name= "csky-crc16-none",
			.cra_priority	= 100,
			.cra_flags	= CRYPTO_ALG_ASYNC,
			.cra_blocksize  = CHKSUM_BLOCK_SIZE,
			.cra_ctxsize	= sizeof(struct csky_crypto_crc_ctx),
			.cra_alignmask  = 0,
			.cra_module	= THIS_MODULE,
			.cra_init	= csky_crypto_crc16none_cra_init
		}
	},
